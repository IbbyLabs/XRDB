...
test('legacy shared iconShape seeds per-type icon shapes', () => {
  const normalized = normalizeSavedUiConfig({
    settings: {
      iconShape: 'squircle',
    },
  });

  assert.equal(normalized.settings.posterIconShape, 'squircle');
  assert.equal(normalized.settings.backdropIconShape, 'squircle');
  assert.equal(normalized.settings.thumbnailIconShape, 'squircle');
  assert.equal(normalized.settings.logoIconShape, 'squircle');
});

test('config payload keeps iconShape per-type when values differ', () => {
  const normalized = normalizeSavedUiConfig({
    settings: {
      tmdbKey: 'tmdb-key-123',
      mdblistKey: 'mdblist-key-456',
      ...
      logoIconShape: 'original',
    },
  });

  const configString = buildConfigString('https://xrdb.example.com', normalized.settings);
  assert.notEqual(configString, '');
  const decodedConfig = JSON.parse(decodeBase64Url(configString));

  assert.equal(decodedConfig.posterIconShape, 'rounded');
  assert.equal(decodedConfig.backdropIconShape, 'circle');
  assert.equal(decodedConfig.thumbnailIconShape, 'squircle');
  assert.equal('logoIconShape' in decodedConfig, false);
  assert.equal('iconShape' in decodedConfig, false);
});
