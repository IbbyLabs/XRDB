...
test('buildDiscordReleasePayload still parses legacy space separated tracked ids', () => {
  const payload = buildDiscordReleasePayload({
    repository: 'IbbyLabs/xrdb',
    previousReleaseTag: 'v2.30.0',
    release: {
      ...
      html_url: 'https://github.com/IbbyLabs/xrdb/releases/tag/v2.31.0',
      published_at: '2026-03-25T01:05:37Z',
      body: ['## Fixed', '* BUG 32 keep legacy spacing readable', '', '## Added', '* FR 21 keep legacy spacing readable'].join('\n'),
    },
  });

  const sectionFields = payload.embeds[0].fields.slice(3);
  assert.equal(sectionFields[0].value, '• FR 21 keep legacy spacing readable');
  assert.equal(sectionFields[1].value, '• BUG 32 keep legacy spacing readable');
});
