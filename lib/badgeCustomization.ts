...
export const MAX_GENRE_BADGE_BORDER_WIDTH_PX = 6;
export const DEFAULT_POSTER_GENRE_BADGE_BORDER_WIDTH_PX = 1.4;
...
export const DEFAULT_NO_BACKGROUND_BADGE_OUTLINE_COLOR = '#000000';
...
export const QUALITY_BADGE_OPTIONS: Array<{ id: MediaFeatureBadgeKey; label: string }> = [
  ...
  { id: 'trendingtoday', label: 'Trending Today' },
  { id: 'trendingweek', label: 'Trending This Week' },
  { id: 'top10', label: 'Top 10' },
  { id: 'top25', label: 'Top 25' },
  { id: 'bingeready', label: 'Binge Ready' },
  { id: 'fanfavourite', label: 'Fan Favourite' },
  { id: 'toprated', label: 'Top Rated' },
  { id: 'oscarwinner', label: 'Oscar Winner' },
  { id: 'oscarnominee', label: 'Oscar Nominee' },
  { id: 'emmywinner', label: 'Emmy Winner' },
  { id: 'emmynominee', label: 'Emmy Nominee' },
...
export const normalizeQualityBadgeScalePercent = (
  value: unknown,
  fallback = DEFAULT_BADGE_SCALE_PERCENT,
) => {
  const numericValue =
...
export const normalizeGenreBadgeBorderWidthPx = (
  value: unknown,
  fallback = DEFAULT_POSTER_GENRE_BADGE_BORDER_WIDTH_PX,
) => {
  const numericValue =
    typeof value === 'number'
      ? value
...
export const normalizeNoBackgroundBadgeOutlineWidthPx = (
  value: unknown,
  fallback = DEFAULT_NO_BACKGROUND_BADGE_OUTLINE_WIDTH_PX,
) => {
  const numericValue =
    typeof value === 'number'
      ? value
  ...
  return Math.max(
    ...
    Math.min(MAX_NO_BACKGROUND_BADGE_OUTLINE_WIDTH_PX, Math.round(numericValue * 10) / 10),
  );
};
