...
| `XRDB_LABEL_LANGUAGES_DIR` | unset | A directory of genre label language files, so anyone can add a language without waiting for a release. One JSON file per language named for its code — `pt.json` — mapping each genre family id to its label; copy the shipped `en.json` for the full set of ids. A file need not be complete, since a family it does not name keeps its English label, so a partial translation is still worth adding. A file here replaces a shipped label of the same name. Labels are stored in their natural case and the badge's own casing control decides how they are drawn. |
