...
| `XRDB_MDBLIST_BULK_RESERVE_PCT` | `25` | The share of MDBList's daily allowance a catalogue sweep may not spend, as a percentage. A pre-warm of a large library can spend a day's allowance in one run and leave nothing for the renders people are waiting on; past the cut-off a sweep is served the rating it was last given rather than asking again, and interactive renders keep the remainder. `0` lets sweeps spend all of it. A render carrying the owner's own MDBList key is unaffected, since it has its own allowance. |
