...
| `XRDB_STREAM_ADDON_URL` | unset | Stremio stream addon asked which qualities a title actually has a release in, so the `Only show what's available` switch can drop the quality badges it has none for. Takes a base URL or a full `/manifest.json` install link; Comet (`http://comet:2020`) and Torrentio (`https://torrentio.stremio.ru`) both work. A configured link carrying a debrid token narrows the answer to that account, so it is treated as a secret and only its host is ever logged. Unset leaves quality badges as the plain labels they are. |
