...
func TestARefusedCallerDoesNotTakeASlot(t *testing.T) {
	...
	_, _, _, _ = p.reserve(CallerInteractive, 0, false, p.maxWait) // slot at now
	_, _, _, _ = p.reserve(CallerInteractive, 0, false, p.maxWait) // slot at +1s, still inside the budget

	before := p.next
	if _, _, _, err := p.reserve(CallerInteractive, 0, false, p.maxWait); !errors.Is(err, ErrPacerBacklog) {
...
func TestThePacerRefusesATurnItCannotUse(t *testing.T) {
	...
	if _, _, _, err := p.reserve(CallerInteractive, 30*time.Second, true, p.maxWait); err != nil {
		t.Errorf("refused a turn with 30s of budget: %v", err)
	}

	// Not enough left for the call: refuse rather than consume it.
	if _, _, _, err := p.reserve(CallerInteractive, time.Second, true, p.maxWait); !errors.Is(err, ErrPacerBacklog) {
...
func TestABudgetAlreadySpentIsRefusedNotIgnored(t *testing.T) {
	...
	if _, _, _, err := p.reserve(CallerInteractive, -500*time.Millisecond, true, p.maxWait); !errors.Is(err, ErrPacerBacklog) {
