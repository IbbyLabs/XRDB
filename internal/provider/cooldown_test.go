...
func TestRateLimitPutsASourceInCooldown(t *testing.T) {
	...
	if h.CoolingOff("simkl") {
		t.Fatal("a source starts available")
	}
	h.Failure("simkl", &RateLimitError{Source: "simkl", RetryAfter: 5 * time.Second, Status: 429})
	if !h.CoolingOff("simkl") {
...
func TestOrdinaryFailureDoesNotCoolOff(t *testing.T) {
	h := NewHealthTracker(10, time.Hour)
	h.Failure("omdb", errors.New("omdb: http 401"))
	if h.CoolingOff("omdb") {
...
func TestRefusalWithoutRetryAfterStillCoolsOff(t *testing.T) {
	h := NewHealthTracker(10, time.Hour)
	h.Failure("simkl", &RateLimitError{Source: "simkl", Status: 429})
	if !h.CoolingOff("simkl") {
...
func TestCooldownIsCapped(t *testing.T) {
	h := NewHealthTracker(10, time.Hour)
	h.Failure("simkl", &RateLimitError{Source: "simkl", RetryAfter: time.Hour, Status: 429})
	h.mu.Lock()
...
func TestCooldownExpires(t *testing.T) {
	h := NewHealthTracker(10, time.Hour)
	h.Failure("simkl", &RateLimitError{Source: "simkl", RetryAfter: time.Millisecond, Status: 429})
	deadline := time.Now().Add(2 * time.Second)
	for h.CoolingOff("simkl") && time.Now().Before(deadline) {
