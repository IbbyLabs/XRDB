...
func TestAnEntryNearExpiryIsRefreshedBehindTheRender(t *testing.T) {
	...
	meta, err := c.do(t.Context(), "k", titleAge{}, func(context.Context) (*provider.MediaMeta, bool, error) {
		calls.Add(1)
		close(done)
...
func TestAnEntryWellInsideItsTermIsNotRefreshed(t *testing.T) {
	...
	var calls atomic.Int32
	for range 5 {
		if _, err := c.do(t.Context(), "k", titleAge{}, func(context.Context) (*provider.MediaMeta, bool, error) {
			calls.Add(1)
			return ratedAt(2), true, nil
		}); err != nil {
			t.Fatal(err)
...
func TestARefreshOutlivesTheRenderThatTriggeredIt(t *testing.T) {
	...
	if _, err := c.do(ctx, "k", titleAge{}, func(fctx context.Context) (*provider.MediaMeta, bool, error) {
...
func TestAFailedRefreshIsLogged(t *testing.T) {
	...
	if _, err := c.do(t.Context(), key, titleAge{}, func(context.Context) (*provider.MediaMeta, bool, error) {
		defer close(done)
