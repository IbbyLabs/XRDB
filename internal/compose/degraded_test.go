...
func degradedFor(t *testing.T, p *Pipeline, req Request) bool {
	t.Helper()
	_, _, degraded, _, _, _ := p.collectRatingsWithProviders(context.Background(), req, &provider.MediaMeta{})
	return degraded
}
...
func degradedSourcesFor(t *testing.T, p *Pipeline, req Request) []string {
	t.Helper()
	_, _, _, _, _, sources := p.collectRatingsWithProviders(context.Background(), req, &provider.MediaMeta{})
	return sources
}
