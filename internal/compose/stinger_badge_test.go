...
func TestStingerBadgeDrawsAndEmptyDoesNot(t *testing.T) {
	...
	drawStingerBadge(drawn, provider.StingerInfo{PostCredits: true}, "bl", 1.0, newOccupancy(image.Rect(0, 0, 400, 200)), stingerBadgeOpts{})
	if stingerInk(drawn) == 0 {
		t.Error("a post-credits stinger drew no badge")
	}
	empty := image.NewNRGBA(image.Rect(0, 0, 400, 200))
	drawStingerBadge(empty, provider.StingerInfo{}, "bl", 1.0, newOccupancy(image.Rect(0, 0, 400, 200)), stingerBadgeOpts{})
