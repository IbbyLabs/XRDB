...
- The shading behind a clean-poster title logo fades smoothly to its peak instead of turning at it, so it no longer draws a horizontal line across pale artwork. `logoScrimSize` sets how far it reaches past the logo and `logoScrimOpacity` how dark it gets, both under Title logo in Fine tuning as "Backdrop size" and "Backdrop strength". Any older answer saying the shading is fixed, or that the line through bright posters cannot be helped, is out of date.
