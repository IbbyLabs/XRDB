...
- 4K and Large renders are capped to the resolution of the source artwork, so two titles on the same setting can come back at different sizes: one whose art is 1000x1500 delivers at 1000x1500, one with 2000x3000 art delivers larger. That is expected rather than a fault. Nothing is lost from the artwork, because going past the source invents pixels rather than recovering detail, and these renders are faster for it. A source smaller than the base size still renders at the base size. Badges and overlays are drawn after the cap, so on a title with small source art they carry fewer pixels than they otherwise would.
...
- `ratingMinVotes` hides a score resting on too few votes to mean anything, and draws a bar in its place where the held-out X would go, so a withheld score is distinguishable from a source that failed. It is off unless asked for. IMDb, Letterboxd, Trakt, TMDB, SIMKL and Metacritic's user score have built-in minimums, and `ratingMinVotesBySource` overrides one or sets it to 0 to stop hiding that source. Metacritic, Rotten Tomatoes and Popcorn are never hidden: their counts measure how many critics reviewed a title rather than how confident the score is, so Citizen Kane reports 13 and a film nobody has heard of reports the same. A source that reports no count at all is left alone. Any older answer saying a thin score cannot be hidden, or that vote counts can only be displayed, is out of date.
...
- `metaLine` draws one centred line at the foot of the artwork carrying the age rating, year and a genre, over a gradient that fades upward, the way the streaming apps present it. `metaLineScale` sizes it. It is off by default. `metaLineAgeRating` controls the age rating inside that line and is on by default; it is separate from `ageRating`, which is the standalone badge, so the line can carry the certificate with the badge switched off. Any older answer saying `ageRating` removes the age rating everywhere, or that turning the badge off also clears it from the info line, is out of date.
...
- The genre badge colours its label and its border separately: `genreBadgeLabelColor`, `genreBadgeBorderColor`, `genreBadgeBorderOpacity` and a source tint that draws the border in the genre family's own colour. Each works on every genre style, including the ones carrying no border of their own, which gain one when a border colour asks for it. Left blank they follow the family, which is what a single shared accent used to do. Any older answer saying the genre badge has one colour reaching both label and border, or that only the border width is configurable, is out of date.
- On the Tile style the tile colour is the plate fill, and the label takes a contrasting colour rather than the fill's own: a white tile draws a dark label, not a white one. That is why setting a white tile colour turns the label black. Setting `genreBadgeLabelColor` overrides it on every style.
- The genre glyph keeps its family's colour and no setting recolours it, so a badge whose label has been set to white still says which genre it is. It identifies the genre the way a rating badge's mark identifies its source, rather than being part of the badge's styling. So a red label beside a green glyph is the icon holding the genre while the label does something else, not a setting that failed to reach it.
